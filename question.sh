...
#
sed '/\/20[0-9][0-9];/!s/\/\([0-9][0-9]\);/\/20\1;/ ; /[0-9][0-9]\//!s/\([0-9]\)\//0\1\// ; /\/[0-9][0-9]\//!s/\/\([0-9]\)\//\/0\1\// ; s/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\);/\3-\2-\1;/ ; y/abcn/ABCN/ ; s/;;/;\\N;/ ; s/;N/;\\N/ ; s/,/./ ; s/;/,/g ; s/\(,\)\s*$/,\\N/' data.csv > output.csv